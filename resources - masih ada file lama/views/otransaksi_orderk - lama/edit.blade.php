...
<div class="content-wrapper">
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Edit Order Kerja {{$JUDUL}} {{$header->NO_BUKTI}}</h1>	
            </div>
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{url('/orderk?JNSOK=')}}{{$header->FLAG}}">Transaksi Order Kerja {{$JUDUL}}</a></li>
    ...
    <div class="content">
        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{url('/orderk/update/'.$header->NO_ID)}}" id="entri" method="POST">
                        @csrf
                        ...
                        <div class="tab-content mt-3">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text" class="form-control ID_SOD" id="ID_SOD" name="ID_SOD" value="{{$header->ID_SOD}}" readonly hidden>
                                </div>
                                <div class="col-md-1" align="right">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-2">
                                    <input type="text" onclick="select()" onkeyup="hitung()" class="form-control QTYH" id="QTYH" name="QTYH" {{ ($header->FLAG == "OK") ? 'value='.$header->QTY : 'value='.$header->QTY_BHN }}>
                                </div>
                            </div>
								...
								@foreach ($detail as $orderkd)
                                    <tr>
                                        <td>
                                            ...
                                            <input name="REC[]" id="REC{{$no}}" type="text" value="{{$orderkd->REC}}" class="form-control REC" onkeypress="return tabE(this,event)" style="text-align: center;" readonly>
                                        </td>
                                        <td>
                                            <input name="KD_BHN[]" id="KD_BHN{{$no}}" type="text" class="form-control KD_BHN" placeholder="Bahan#" value="{{$orderkd->KD_BHN}}" readonly required>
                                        </td>
                                        <td>
                                            <input name="NA_BHN[]" id="NA_BHN{{$no}}" type="text" class="form-control NA_BHN" value="{{$orderkd->NA_BHN}}" readonly required>
                                        </td>
                                        <td>
                                            <input name="SATUAN[]" id="SATUAN{{$no}}" type="text" class="form-control SATUAN" placeholder="Satuan" value="{{$orderkd->SATUAN}}" readonly required>
                                        </td>
										<td>
											<input name="QTY[]" onclick="select()" onkeyup="hitung()" id="QTY{{$no}}" type="text" style="text-align: right"  class="form-control QTY text-primary" value="{{$orderkd->QTY}}" readonly> <input name="QTYX[]" id="QTX{{$no}}" type="text" class="form-control QTYX text-primary" value="{{$orderkd->QTYX}}" readonly hidden>
										</td>   
                                        
										<td>
                                            <input name="KET[]" id="KET{{$no}}" type="text" class="form-control KET" placeholder="Ket" value="{{$orderkd->KET}}" required>
                                        </td>
...
// TAMBAH HITUNG
	$(document).ready(function() {
		...
		$(".KD_BHN").each(function() {
			...
			// $("#KD_PRS"+noid).keypress(function(e){
			// 	if(e.keyCode == 46){
			// 		e.preventDefault();
			// 		browsePrs(noid);
			// 	}
...
////////////////////////////////////////////////////////////
		var dTableFo;
		loadDataFo = function(){
			var kode = "{{$header->FLAG}}"=="OK" ? $("#KD_BRG").val() : $("#KD_BHN_H").val();
			$.ajax(
			{
				type: 'GET',    
...
////////////////////////////////////////////////////////////////////////////////////
		...
		$.ajax(
			{
				...
				success: function( resp )
				{
					...
					for(i=0; i<resp.length; i++){
						html+=`<tr>
                                    <td><input name='NO_ID[]' id='NO_ID${i}' type='hidden' class='form-control NO_ID' value='new' readonly> <input name='REC[]' id='REC${i}' value=${resp[i].REC+1} type='text' class='REC form-control' onkeypress='return tabE(this,event)' readonly></td>
	...
	function simpan() {
		...
		if ("{{$header->FLAG}}"=="OW" && $('#KD_BHN_H').val()=="")
		{
			check = '1';
    ...
    function tambah() {
		...
		// td5.innerHTML = "<input name='QTYA[]' onclick='select()' onkeyup='hitung()' value='0' id=QTYA" + idrow + " type='text' style='text-align: right' class='form-control QTYA  text-primary' readonly required>";
