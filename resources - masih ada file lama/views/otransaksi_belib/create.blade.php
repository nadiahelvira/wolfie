...
<div class="content-wrapper">
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
            <h1 class="m-0">Tambah Pembelian Bahan Baku Baru</h1>
            </div>
            <!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="/belib">Transaksi Pembelian Bahan Baku </a></li>
                    ...
                    <form action="store" method="POST">
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="form-group row">
                                ...
                                <div class="col-md-4">
                                    <input type="text" class="form-control NAMAS" id="NAMAS" name="NAMAS" placeholder="NAMAS" readonly>
                                </div>
                            </div>
	...
	<div class="modal fade" id="browseBPodModal" tabindex="-1" role="dialog" aria-labelledby="browseBPodModalLabel" aria-hidden="true">
	  <div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="browsePodModalLabel">Cari Bahan#</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
...
//////////////////////////////////////////////////////////////////////
			...
			$.ajax(
			{
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBPod.row.add([
							'<a href="javascript:void(0);" onclick="chooseBPod(\''+resp[i].KD_BHN+'\',\''+resp[i].NA_BHN+'\', \''+resp[i].SATUAN+'\' ,\''+resp[i].QTY+'\' ,\''+resp[i].HARGA+'\' , \''+resp[i].KIRIM+'\' ,\''+resp[i].SISA+'\' )">'+resp[i].KD_BHN+'</a>',							
							resp[i].NA_BHN,
							resp[i].SATUAN,
							resp[i].QTY,
							resp[i].HARGA,
		...
		chooseBPod = function(KD_BHN,NA_BHN,SATUAN,SISA,HARGA){
			$("#KD_BHN"+rowidBPod).val(KD_BHN);
			$("#NA_BHN"+rowidBPod).val(NA_BHN);
			$("#QTY"+rowidBPod).val(SISA);
			$("#SATUAN"+rowidBPod).val(SATUAN);
			$("#HARGA"+rowidBPod).val(HARGA);
 ...
 function simpan() {
    ...
    function tambah() {
		...
		$("#KD_BHN"+idrow).keypress(function(e){
			if(e.keyCode == 46){
				e.preventDefault();
				browseBPod(eval($(this).data("rowid")));
			}
		}); 
