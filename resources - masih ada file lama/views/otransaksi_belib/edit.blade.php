...
<div class="content-wrapper">
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
               <h1 class="m-0">Edit Pembelian {{$header->NO_BUKTI}}</h1>	
            </div>
            <!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{url('/belib')}}">Pembelian</a></li>
    ...
    <div class="content">
        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{url('/belib/update/'.$header->NO_ID)}}" method="POST">
                        @csrf
                        ...
                        <div class="tab-content mt-3">
                            ...
                            <table id="datatable" class="table table-striped table-border">
								...
								<?php $no=0 ?>
								@foreach ($detail as $belid)
                                    <tr>
                                        <td>
										   <input type="hidden" name="NO_ID[]{{$no}}" id="NO_ID" type="text" value="{{$belid->NO_ID}}" 
                                            class="form-control NO_ID" onkeypress="return tabE(this,event)" readonly>
										
                                            <input name="REC[]" id="REC{{$no}}" type="text" value="{{$belid->REC}}" class="form-control REC" onkeypress="return tabE(this,event)" readonly>
                                        </td>
                                        <td>
                                            <input name="KD_BHN[]" id="KD_BHN0" type="text" value="{{$belid->KD_BHN}}" class="form-control KD_BHN" readonly required>
                                        </td>
                                        <td>
                                            <input name="NA_BHN[]" id="NA_BHN0" type="text" value="{{$belid->NA_BHN}}" class="form-control NA_BHN" readonly required>
                                        </td>
                                        <td>
                                            <input name="SATUAN[]" id="SATUAN0" type="text" value="{{$belid->SATUAN}}" class="form-control SATUAN" readonly required>
                                        </td>
										
										<td><input name="QTY[]" onclick="select()" onkeyup="hitung()" value="{{$belid->QTY}}" id="QTY0" type="text" style="text-align: right"  class="form-control QTY text-primary"></td>                         
										<td><input name="HARGA[]" onclick="select()" onkeyup="hitung()" value="{{$belid->HARGA}}" id="HARGA0" type="text" style="text-align: right"  class="form-control HARGA text-primary"></td>
										<td><input name="TOTAL[]" onclick="select()" onkeyup="hitung()" value="{{$belid->TOTAL}}" id="TOTAL0" type="text" style="text-align: right"  class="form-control TOTAL text-primary" readonly></td>
                                        
										<td>
                                            <input name="KET[]" id="KET0" type="text" value="{{$belid->KET}}" class="form-control KET" required>
                                        </td>
	...
	<div class="modal fade" id="browsePodModal" tabindex="-1" role="dialog" aria-labelledby="browsePodModalLabel" aria-hidden="true">
	  <div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="browsePodModalLabel">Cari Barang#</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
		  <div class="modal-body">
			<table class="table table-stripped table-bordered" id="table-bpod">
				<thead>
					<tr>
						<th>Barang#</th>
						<th>-</th>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
	...
	var dTableBPo;
			...
			$.ajax(
			{
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBPo.row.add([
							'<a href="javascript:void(0);" onclick="choosePo(\''+resp[i].NO_BUKTI+'\',  \''+resp[i].KODES+'\', \''+resp[i].NAMAS+'\', \''+resp[i].ALAMAT+'\', \''+resp[i].KOTA+'\' , \''+resp[i].KD_BHN+'\' , \''+resp[i].NA_BHN+'\' , \''+resp[i].HARGA+'\', \''+resp[i].QTY+'\', \''+resp[i].KIRIM+'\', \''+resp[i].SISA+'\'  )">'+resp[i].NO_BUKTI+'</a>',
							resp[i].KODES,
							resp[i].NAMAS,
							resp[i].ALAMAT,
							resp[i].KOTA,
		...
		choosePo = function(NO_BUKTI,KODES,NAMAS,ALAMAT, KOTA, KD_BHN, NA_BHN, HARGA, KG, KIRIM, SISA){
			$("#NO_PO").val(NO_BUKTI);
			$("#KODES").val(KODES);
			$("#NAMAS").val(NAMAS);
			$("#ALAMAT").val(ALAMAT);
...
//////////////////////////////////////////////////////////////////////
 		...
 		var dTableBPod;
		var rowidBPod;
		loadDataBPod = function(){
			$.ajax(
			{
				type: 'GET',    
				url: "{{url('pob/browse_detail')}}",
				data: {
					'NO_PO': $("#NO_PO").val(),
				},
				success: function( response )
		...
		dTableBPod = $("#table-bpod").DataTable({
			
		});
		
		browseBPod = function(rid){
			rowidBPod = rid;
			loadDataBPod();
			$("#browseBPodModal").modal("show");
		}
		
		chooseBPod = function(KD_BHN,NA_BHN,SISA,HARGA){
			$("#KD_BHN"+rowidAccount).val(KD_BHN);
			$("#NA_BHN"+rowidAccount).val(NA_BHN);
			$("#QTY"+rowidAccount).val(SISA);
			$("#HARGA"+rowidAccount).val(HARGA);
			$("#browseBPodModal").modal("hide");
			hitung();
		}
...
//////////////////////////////////////////////////////////////////
 ...
 function simpan() {
    ...
    function tambah() {
        ...
        td4.innerHTML = "<input name='SATUAN[]'   id=SATUAN" + idrow + " type='text' class='form-control  SATUAN' required readonly>";
