...
class ExportXLS{
	...
	//echo "3";
	//echo "JUMLAH : ".count($raw->arrayband)."<BR>";
$i = 0;
	...
	//echo "5b";
                            $this->title();	//disini ada error (error pas evaluate) 
	//echo "5c";
					//$this->maxrow--;
					$this->relativey = $this->relativey+1;
	//echo "5d";
...
foreach($this->arraytitle as $out){
    ...
    $this->currentband="pageHeader";
      ...
      $this->maxrow=$this->headerrowcount;
		//echo " H4 : ".count($this->arraypageHeader);
		//echo "<pre>";
		//print_r($this->arraypageHeader);
		//echo "</pre>";
        foreach($this->arraypageHeader as $i => $out){
			//echo " H5 ($i) ";
            $this->display($out,0);
...
$r=0;
      foreach($this->arraydetail as $detail){
          $detailheight= $this->arrangeRows($detail);
			...
			//btr bos  tadi aku liat filesize antara sample4 dan sample4 (1) beda 2 kb bos..
			//oya hmm tp sama2 ga kluar kan ya di excel
	...
	echo "</pre><br>";*/
      ...
      if($this->debughtml){
        ...
        switch($arraydata['type']){
            case "MultiCell":       
			//echo "MC1 ";
			//masalahnya disini relativey nya pdhal diisi tp selalu balik kosong hmm
			//echo "Relative Y 1 : [".$this->relativey."] ";
                if(!isset($this->relativey) || $this->relativey==""){
                    $this->relativey=1;
				}    
				//echo "Relative Y 2 : [".$this->relativey."] ";
			//echo "MC2 ";
                   //$this->mergeCells(    $this->relativex,  ($this->relativey+$rowpos),   ($this->cols['c'.($this->mergex+$arraydata['width'])]-1),   ($this->relativey+$rowpos)  );
          
			//echo "MC3 : $arraydata[txt] ";
			...
			//echo "MC5 : Set Text: Posisi ".$this->relativex.",".($this->relativey+$rowpos)." | Isi : ".$txt."<br/>";
            ...
            case "SetXY":
				...
				if(isset($this->rows['r'.$myy]) && $this->rows['r'.$myy] != ""){
					$this->relativey=$this->rows['r'.$myy];
					//echo " Set Relative Y : ".$this->rows['r'.$myy]."<br>";
				}
                $this->mergex=$myx;
       ...
       foreach($this->arrayParameter as  $pv => $ap) {
           ...
           if(is_numeric($ap)&&$ap!=''&& ($this->left($ap,1)>0 || $this->left($ap,1)=='-')){
            ...
            $fm = str_replace('$P_'.$pv.$backcurl, "'".$ap."'",$fm);
             ...
             //echo "AE 12 (".count($this->arrayfield).") ";
       //     print_r($this->arrayfield);
       foreach($this->arrayfield as $afi => $af){
		   //echo " AE13 ($afi)[$af] ";
...
//              $fm=str_replace('+',".",$fm);
        ...
        if((strpos('"',$fm)==false) || (strpos("'",$fm)==false)){
			 ...
			 if(is_numeric($fm) == false && strpos("'",$fm)==false){
				 $fm = "'".$fm."'";
			 }
			  $fm=str_replace("''", "'", $fm);
            //echo ' AE 23 <b> $result= '.$fm.' </b> <br/>';    
